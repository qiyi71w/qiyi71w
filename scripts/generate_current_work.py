...
def search(query, token, sort=None, order=None, per_page=1):
    ...
    if order:
        params["order"] = order
    return api_get("/search/issues?" + urllib.parse.urlencode(params), token)
...
def contributor_data(item, token):
    ...
    recent = search(f"repo:{repo} is:pr author:{user}", token, "created", "desc", 1)
    latest = None
    if recent.get("items"):
...
def demo_repo(item):
    return {
        ...
        "status": "Active",
        "focus": "Current development",
        "description": "Desktop software and developer tooling.",
        "language": item.get("stack") or "C++",
        "stars": 18,
        "issues": 4,
        "prs": 1,
        "updated": "1d ago",
...
def demo_contributor(item):
    return {
        ...
        "focus": "Contributing via pull requests",
        "contributor": item.get("contributor", "qiyi71w"),
        "merged": 79,
        "open": 0,
        "latest": {
            "number": 367,
            "title": "fix(engine): converge switch menu to PRIMARY after failed rollback (#366)",
            "url": "https://github.com/wimi321/lizzieyzy-next/pull/367",
            "state": "Merged",
            "updated": "just now",
        },
    }
...
def render_contributor(d, y):
    latest = d.get("latest")
    if latest:
        line = f'#{latest["number"]}  {latest["title"]}'
        url = latest["url"]
        meta = f'{latest["state"]} · {latest["updated"]}'
    else:
...
def svg(cards, dark=False):
    y = 8
    chunks = []
    for card in cards:
        if card["mode"] == "contributor":
            chunks.append(render_contributor(card, y))
            y += 192
        else:
            chunks.append(render_repo(card, y))
            y += 146
    height = y + 10
    c = palette(dark)
    body = "".join(chunks)
...
.latest{{fill:{c["latest"]};stroke:{c["border"]};stroke-width:1}}
.repo{{fill:{c["blue"]};font:600 16px Arial,sans-serif}}
.status{{fill:{c["muted"]};font:600 12px Arial,sans-serif}}
.focus{{fill:{c["text"]};font:600 14px Arial,sans-serif}}
.desc{{fill:{c["muted"]};font:400 12px Arial,sans-serif}}
.meta{{fill:{c["muted"]};font:400 11px Consolas,monospace}}
.latest-label{{fill:{c["muted"]};font:700 9px Arial,sans-serif;letter-spacing:.12em}}
.latest-title{{fill:{c["text"]};font:600 12px Arial,sans-serif}}
.green{{fill:{c["green"]}}}
</style>
...
def main():
    ...
    parser.add_argument("--output", default="assets/current-work.svg")
    parser.add_argument("--dark-output", default=None)
    ...
    output.write_text(svg(cards, dark=False), encoding="utf-8")

    if args.dark_output:
        dark_output = Path(args.dark_output)
        dark_output.parent.mkdir(parents=True, exist_ok=True)
        dark_output.write_text(svg(cards, dark=True), encoding="utf-8")
